...
export function ErrorModal({ isOpen, onClose, error }) {
 ...
 <div className="p-2 rounded-full shrink-0 bg-rose-900/30 text-rose-400">
 <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
 </svg>
 </div>
 <div className="flex-1">
 <h3 className="text-lg font-bold text-gray-100">Error</h3>
 <p className="text-sm text-rose-300 mt-1 whitespace-pre-wrap">{error}</p>
 </div>
 </div>
...
export function ConfirmModal({ isOpen, onClose, onConfirm, title, message, confirmText = 'Confirmar', cancelText = 'Cancelar', variant = 'danger' }) {
 ...
 <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={variant === 'danger' ? 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z' : 'M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'} />
 </svg>
 </div>
 <div className="flex-1">
 <h3 className="text-lg font-bold text-gray-100">{title}</h3>
 ...
 variant === 'danger' ? '!bg-red-600 hover:!bg-red-500 shadow-lg shadow-red-600/30' : variant === 'warning' ? '!bg-amber-600 hover:!bg-amber-500 shadow-lg shadow-amber-600/30' : 'shadow-lg shadow-brand-600/30'
 }`}
 >
 {confirmText}
 </button>
...
export function DetailModal({ isOpen, onClose, title, fields }) {
 ...
 <p ref={idx === 0 ? initialRef : undefined} tabIndex={idx === 0 ? 0 : -1} className="text-sm text-gray-200 whitespace-pre-wrap break-words bg-dark-600/50 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-brand-500/40">{field.value || '—'}</p>
 </div>
 ))}
 </div>
 <div className="flex justify-end mt-6 pt-4 border-t border-dark-400">
